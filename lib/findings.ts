...
const slVsTp: Detector = (trades, m) => {
  ...
  return {
    ...
    stat: { value: `${ratio.toFixed(1)}×`, label: "stops hit vs targets hit" },
...
const manualExits: Detector = (trades, m) => {
  ...
  return {
    ...
    stat: { value: `${man.winRate.toFixed(0)}%`, label: `win rate on ${man.count} manual closes` },
...
const concentration: Detector = (trades, m) => {
  ...
  return {
    ...
    stat: { value: pct(share), label: `of trades in ${top[0]}` },
...
const stacked: Detector = (trades, m) => {
  ...
  return {
    ...
    stat: { value: pct(share), label: "of trades opened in clusters" },
...
const worstSession: Detector = (trades, m) => {
  ...
  return {
    ...
    stat: { value: `${ws.winRate.toFixed(0)}%`, label: `win rate in ${worst === "ny" ? "New York" : worst}, your weakest session` },
...
const worstHour: Detector = (trades, m) => {
  ...
  return {
    ...
    stat: { value: money(v.pnl), label: `from ${v.count} trades at ${h}:00` },
...
const noStop: Detector = (trades, m) => {
  ...
  return {
    ...
    stat: { value: pct(share), label: "of trades without a stop-loss" },
...
const revenge: Detector = (trades, m) => {
  ...
  return {
    ...
    stat: { value: String(ids.length), label: `re-entries within 5 min of a loss, ${money(pnl)}` },
...
const holdAsymmetry: Detector = (trades, m) => {
  ...
  return {
    ...
    stat: { value: `${(avgLossMins / avgWinMins).toFixed(1)}×`, label: "longer hold on losers than winners" },
...
const worstDay: Detector = (trades, m) => {
  ...
  return {
    ...
    stat: { value: money(pnl), label: `on ${day}, ${count} trades` },
...
const drawdown: Detector = (_trades, m) => {
  ...
  return {
    ...
    stat: { value: pct(ratio * 100), label: "of peak profit given back" },
